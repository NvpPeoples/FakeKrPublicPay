...
<div class="schools-search">
    <div class="row lead">
        <div class="col-sm-6">
            <dl>
                <dt><?= $school->getAttributeLabel('boss')?></dt>
                <dd><?= Html::encode($school->boss)?></dd>
            </dl>
            <dl>
                <dt><?= $school->getAttributeLabel('code_reg')?></dt>
                <dd><?= Html::encode($school->region->name_reg)?></dd>
            </dl>
        </div>
        <div class="col-sm-6">
                <dt><?= $school->getAttributeLabel('phone')?></dt>
                <dd><?= $school->phone?></dd>
        </div>
    </div>
</div>
<div class="pays-index">
    ...
    <div class="row">
        ...
        <div class="col-sm-4">
            ...
            <br/>
            <div class="text-center">Заплановано: <?= Yii::$app->formatter->asCurrency($school->getLimitOfYear($row['year']))?></div>
            <?php
                $inst = $gauge;
                $inst['yAxis']['max'] = round($school->getLimitOfYear($row['year']));
                if ($inst['yAxis']['max'] > 0) {
            ...
            <p class="text-center"><?= Html::a('переглянути <i class="glyphicon glyphicon-chevron-right"></i>', [
                'pays', 'ow' => $school->id, 'PaysOnSchoolSearch[year]' => $row['year']
            ], [
