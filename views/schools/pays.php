...
$this->params['breadcrumbs'][] = [
    'label' => \yii\helpers\StringHelper::truncate($school->name, 70),
    'url'   => ['detail', 'id' => $school->id]
];
...
<div class="pays-index">
    ...
    <?php
        if ($search->year == date('Y')) {
            $inst = $gauge;
            $inst['yAxis']['max'] = round($school->getLimitOfYear($search->year));
            if ($inst['yAxis']['max'] > 0) {
                $inst['series'][0]['data'][0] = $school->getSumOfCurYearPays();
            } else {
...
<?php
$url_tmpl = Url::to(['/pays/ajax-detail', 'id' => '_id_', 't' => 's']);
